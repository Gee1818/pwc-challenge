"""
LGBM log regression model training script.
"""
...
def main():
    """Main training pipeline for LGBM  model with log transformed target."""
    print("Starting lgbm_log model training pipeline...")
    ...
    best_params = {'max_depth': 4, 'min_data_in_leaf': 30, 'n_estimators': 432, 'learning_rate': 0.03190066200763102, 'feature_fraction': 0.6575124423707558, 'max_bin': 395, 'num_leaves': 16}
            
    # Train lgbm model
    print(f"\nTraining lgbm_log model with params={best_params}...")
    model_lgbm = LGBMRegressor(**best_params, linear_tree=True, random_state=SEED, n_jobs=-1, verbose=-1)
    results_lgbm, importances_lgbm, oof_preds_lgbm_log = cross_validation(model_lgbm, X, y)
    oof_preds_lgbm_log = np.expm1(oof_preds_lgbm_log)
    ...
    bootstrap_ci = bootstrap_confidence_intervals(np.array(y_base), oof_preds_lgbm_log)
    ...
    final_lgbm_log_model = LGBMRegressor(**best_params, linear_tree=True, random_state=SEED, n_jobs=-1, verbose=-1)
    final_lgbm_log_model.fit(X_train, y)
    
    # Save model and predictions
    save_model_and_pipeline(final_lgbm_log_model, preprocessing_pipeline, 'lgbm_log')
    save_oof_predictions(ids, oof_preds_lgbm_log, 'lgbm_log')
    
    print("\nLGBM with log target model training completed successfully!")
